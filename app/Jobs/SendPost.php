...
class SendPost implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $response = Http::timeout($timeout)->retry($retries, $waiting)->get($this->url);
        $log = Log::insert([
        ...
        if ($response->successful()){
            ...
            // ----------------------------------------------------------------------------------------------------------------------
            // This portion of code handle the question 5:
            //
            // 5. Let's say the post was active and it was not failing, What would be the implementation if there was 100K requests?
            //
        ...
        // In this regard, it is importa nt to consider re-gluing it with higher priority to be processed before the subsequent jobs.
        if ($response->failed()){
            $log = Log::insert([
                'batchId'   => $this->batch()->id,
            ...
            // ----------------------------------------------------------------------------------------------------------------------
            // This portion of code handle the question 4:
            //
            ...
            if ($this->batch()->pendingJobs <= $maximumPendingJobsOnQueue){
                ...
                $batch->add(new SendPost($this->url,$this->highPriorityBatchId));
            }
